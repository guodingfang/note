...
正则表达式之所以强大，是因为其能实现模糊匹配。模糊匹配，有两个方向上的模糊：横向模糊和纵向模糊。

### 横向模糊

横向模糊指的是，一个正则可匹配的字符串的长度不是固定的，可以是多种情况。

其实现方式是使用量词，如：`{m,n}`，表示连续出现最少m次，最多n次。
...
> 注意：案例中用的正则表达式时`/ab{2,5}c/g`，后面多个g，它是正则表达式的一个修饰符。表示全局匹配，即在目标字符串中按顺序找到满足匹配模式的所有子串。
...
需要强调的是，虽叫字符组（字符类），但只是其中一个字符。例如`[abc]`，表示一个字符，可以是a，b，c之一。
