...
选择器 | 类型 | 功能描述
---|---|---
E F | 后代选择器 | 匹配E元素内匹配的所有F元素
E>F | 子选择器 | 匹配E元素内匹配的所有子元素F
E+F | 相邻兄弟选择器 | 匹配E元素后面紧跟着的兄弟F元素
E~F | 通用选择器 | 匹配E元素后面所有的兄弟F选择器
...
E F:nth-last-child(n) | 选择父元素E的倒数第n个子元素 F，与 `:nth-child(n)` 正好相反，用法是一样的
E:nth-of-type(n) | 选择父元素内具有指定类型第n个E元素
E:nth-last-of-type(n) | 选择父元素内指定类型的倒数第n个E元素
E:first-of-type | 选择父元素内具有指定类型的第一个E元素，与 `E:nth-of-type(1)`
